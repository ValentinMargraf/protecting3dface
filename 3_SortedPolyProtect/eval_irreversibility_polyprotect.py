...
from helpers.invert_polyprotect_vedrana_single import invert_P, system
...
# inside the levenberg marquardt algorithm
scale = False
...
def main():
    ...
    print("SETTING: \nembeddings scaled: ",scale,"\nexact jacobians used: ",use_exact_jacobians,"\n")
    ...
    for i in range(NUM_ITER):
        if REPRODUCE:
            C=np.load("params/C_pp_"+str(i)+".npy")
            print("params/C_pp_"+str(i)+".npy loaded")
            E=np.load("params/E_pp_"+str(i)+".npy")
            print("params/E_pp_"+str(i)+".npy loaded")
        else:
            # compute parameters C and E, each iteration a different set
            C,E = generate_params(num_persons,5,SortedRadicals=False)
        ...
        for index in range(V.shape[0]):
            ...
            E_ = E[labels[index]]
            C_ = C[labels[index]]
            sol, cosine = invert_P(overlap, V_, P_, C_, E_, num_guesses, el_mins, el_maxs, el_probabilities, precision,use_exact_jacobians)
